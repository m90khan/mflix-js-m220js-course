...
;(async () => {
  try {
    ...
    console.log(host, mflix, "connected")
    ...
    let col = mflix.collection("movies")
    await col.bulkWrite(moviesToMigrate)

    console.log("\x1b[32m", `${moviesToMigrate.length} documents updated`)
    client.close()
    process.exit(0)
  } catch (e) {
    if (
